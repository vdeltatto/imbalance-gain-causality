...
seed = int(sys.argv[1])
traj = pickle.load(open(f"/scratch/vdeltatt/imbalance-gain-causality/fig3_errors/trajs/rossler_diff/seed{seed}_ieps0.p","rb"))
N = 5000
tau = 30
k = 20
n_jobs = 8
...
Es = np.arange(1,50)
tau_e = 1
sample_times = np.linspace(100000,200001,N,dtype=int)
...
for iE, E in enumerate(Es):
    ...
    Xtau = construct_time_delay_embedding(X=traj[:,1], E=E, tau_e=tau_e, sample_times=sample_times+tau)
...
pickle.dump(info_imbalances, open(f"./pickles/rossler_diff/seed{seed}.p","wb"))