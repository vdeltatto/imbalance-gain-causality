...
for i_tau, tau in tqdm(enumerate(taus)):
    ...
    TEs_X_to_Y_givenZ[i_tau] = (
        knncmi.cmi(['x_0'], ['y_tau'], ['y_0','z0'], k=args.k, data=dataset, minzero=args.minzero)
    )
    TEs_Y_to_X_givenZ[i_tau] = (
        knncmi.cmi(['y_0'], ['x_tau'], ['x_0','z0'], k=args.k, data=dataset, minzero=args.minzero)
    )


# save data
pickle.dump([taus, TEs_X_to_Y, TEs_Y_to_X, TEs_X_to_Y_givenZ, TEs_Y_to_X_givenZ], 
            open(f"./pickles_te_longtimes/seed{args.seed}_N{args.N}_k{args.k}.p","wb"))