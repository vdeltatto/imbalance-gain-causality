...
def target_energy_python(states):
    gauss1 = 1/np.sqrt(2*np.pi) * torch.exp(-0.5*(states[0]**2 + states[1]**2 + states[2]**2))
    gauss2 = 1/np.sqrt(2*np.pi) * torch.exp(-0.5*((states[0]-5)**2 + (states[1]-5)**2 + (states[2]-5)**2))
    gauss3 = 1/np.sqrt(2*np.pi) * torch.exp(-0.5*((states[0]-12)**2 + states[1]**2 + (states[2]-12)**2))
    gauss4 = 1/np.sqrt(2*np.pi) * torch.exp(-0.5*((states[0]-12)**2 + states[1]**2 + (states[2]-20)**2))
...
def main():
    ...
    states_start = np.column_stack((
        ...
        np.random.uniform(-3,23,size=(args.N,))
    ))
