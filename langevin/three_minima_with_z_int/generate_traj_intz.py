...
def target_energy_python(states):
    ...
    gauss2 = 1/np.sqrt(2*np.pi) * torch.exp(-0.5*((states[0]-5)**2 + (states[1]-5)**2 + (states[2]-5)**2)) # (5,5,5)
    gauss3 = 1/np.sqrt(2*np.pi) * torch.exp(-0.5*((states[0]-12)**2 + states[1]**2 + (states[2]-16)**2))   # (0,12,16)
