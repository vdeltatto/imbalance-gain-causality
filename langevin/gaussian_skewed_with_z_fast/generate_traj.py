...
def main():
    ...
    parser.add_argument("-friction_z", "--friction_z", dest="friction_z",
                        default=0.1, type=float, help="Friction of Z")
    ...
    # save trajectory
    traj = np.array(traj)
    with open(f"./pickles_traj/seed{args.seed}_N{args.N}_dt{args.dt}_samplingdt{args.sampling_stride}_nsteps{args.n_steps}_frictx{args.friction_x}_fricty{args.friction_y}_frictz{args.friction_z}_temp{args.temperature}.p","wb") as f:
