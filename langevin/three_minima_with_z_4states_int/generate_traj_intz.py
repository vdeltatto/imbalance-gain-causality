...
def target_energy_python(states):
    ...
    gauss2 = 1/(2*np.pi)**(3/2) * torch.exp(-0.5*((states[0]-5)**2 + (states[1]-5)**2 + (states[2])**2))     # (5,5,0)
    gauss3 = 1/(2*np.pi)**(3/2) * torch.exp(-0.5*((states[0]-12)**2 + states[1]**2 + (states[2])**2))        # (12,0,0)
    gauss4 = 1/(2*np.pi)**(3/2) * torch.exp(-0.5*((states[0]-12)**2 + (states[1]-0)**2 + (states[2]-16)**2)) # (12,0,16)
    energy = -torch.log(1/3*gauss1 + 1/3*gauss2 + 1/18*gauss3 + 5/18*gauss4)
    return energy
...
def main():
    ...
    # initialize states
    weights = [1/3, 1/3, 1/18, 5/18]  # Mixture weights
    means = [[0, 0, 0], [5, 5, 0], [12, 0, 0], [12, 0, 16]]  # Means of the components
    ...
    # intervention do(Z=args.int_value)
    states_int = +thermalized_states
    states_int[:,2] = args.int_value
    ...
    # save trajectory
    with open(f"./pickles_traj_intz/intvalue{args.int_value}_seed{args.seed}_N{args.N}_dt{args.dt}_samplingdt{args.sampling_stride}_nsteps{args.n_steps}_frict{args.friction}_temp{args.temperature}.p","wb") as f:
