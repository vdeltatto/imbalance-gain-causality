...
def target_energy_python(states):
    ...
    gauss3 = 1/(2*np.pi)**(3/2) * torch.exp(-0.5*((states[0]-12)**2 + states[1]**2 + (states[2]-12)**2))   # (0,12,12)
    gauss4 = 1/(2*np.pi)**(3/2) * torch.exp(-0.5*(states[0]**2 + states[1]**2 + (states[2]-19)**2))        # (0,0,19)
    energy = -torch.log(1/6*gauss1 + 1/3*gauss2 + 1/3*gauss3 + 1/6*gauss4)
    return energy
...
def main():
    ...
    # initialize states
    weights = [1/6, 1/3, 1/3, 1/6]  # Mixture weights
    means = [[0, 0, 0], [5, 5, 5], [12, 0, 16], [0, 0, 19]]  # Means of the components
