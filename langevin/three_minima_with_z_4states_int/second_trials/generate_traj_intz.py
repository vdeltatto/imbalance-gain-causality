...
def target_energy_python(states):
    ...
    gauss4 = 1/(2*np.pi)**(3/2) * torch.exp(-0.5*((states[0]-5)**2 + (states[1]-5)**2 + (states[2]-22)**2)) # (5,5,22)
    energy = -torch.log(1/3*gauss1 + 1/6*gauss2 + 1/3*gauss3 + 1/6*gauss4)
    return energy
...
def main():
    ...
    means = [[0, 0, 0], [5, 5, 5], [12, 0, 12], [5, 5, 22]]  # Means of the components
