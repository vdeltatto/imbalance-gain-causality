...
def target_energy_python(states):
    gauss1 = 1/np.sqrt(2*np.pi) * torch.exp(-0.5*(states[0]**2 + states[1]**2 + states[2]**2))             # (0,0,0)
    gauss2 = 1/np.sqrt(2*np.pi) * torch.exp(-0.5*((states[0]-5)**2 + (states[1]-5)**2 + (states[2])**2)) # (5,5,0)
    gauss3 = 1/np.sqrt(2*np.pi) * torch.exp(-0.5*((states[0]-12)**2 + states[1]**2 + (states[2])**2))   # (0,12,0)
...
def main():
    ...
    means = [[0, 0, 0], [5, 5, 0], [12, 0, 0]]  # Means of the components
