...
def main():
    ...
    traj_int0 = ULA_update_faster(states_old=states_int, 
                             target_energy=target_energy, 
    ...
    traj_int0 = np.array(traj_int0)

    # intervention do(Y=5)
    states_int = +thermalized_states
    states_int[:,1] = 5.0
    states_int = torch.tensor(states_int)
    traj_int5 = ULA_update_faster(states_old=states_int, 
                             target_energy=target_energy, 
                             n_steps=args.n_steps, 
                             dt=args.dt, 
                             temperature=args.temperature, 
                             ...
                             seed=args.seed,
                             sampling_stride=args.sampling_stride)
    traj_int5 = np.array(traj_int5)'''

    # intervention do(Y=12)
    states_int = +thermalized_states
    states_int[:,1] = 12.0
    states_int = torch.tensor(states_int)
    traj_int12 = ULA_update_faster(states_old=states_int, 
                             target_energy=target_energy, 
                             n_steps=args.n_steps, 
                             dt=args.dt, 
                             temperature=args.temperature, 
                             ...
                             seed=args.seed,
                             sampling_stride=args.sampling_stride)
    traj_int12 = np.array(traj_int12)

    with open(f"./pickles_traj_inty_old/seed{args.seed}_N{args.N}_dt{args.dt}_samplingdt{args.sampling_stride}_nsteps{args.n_steps}_frict{args.friction}_temp{args.temperature}.p","rb") as f:
        traj_int0, traj_int5 = pickle.load(f)

    # save trajectory
    with open(f"./pickles_traj_inty/seed{args.seed}_N{args.N}_dt{args.dt}_samplingdt{args.sampling_stride}_nsteps{args.n_steps}_frict{args.friction}_temp{args.temperature}.p","wb") as f:
        pickle.dump([traj_int0, traj_int5, traj_int12], f)
