...
def main():
    ...
    traj_int3 = ULA_update_faster(states_old=states_int, 
                             target_energy=target_energy, 
    ...
    traj_int3 = np.array(traj_int3)

    # intervention do(X=-3)
    states_int = +thermalized_states
    states_int[:,0] = -3.0
    states_int = torch.tensor(states_int)
    traj_int3neg = ULA_update_faster(states_old=states_int, 
                             target_energy=target_energy, 
                             n_steps=args.n_steps, 
                             dt=args.dt, 
                             temperature=args.temperature, 
                             ...
                             seed=args.seed,
                             sampling_stride=args.sampling_stride)
    traj_int3neg = np.array(traj_int3neg)

    # save trajectory
    with open(f"./pickles_traj_intx/seed{args.seed}_N{args.N}_dt{args.dt}_samplingdt{args.sampling_stride}_nsteps{args.n_steps}_frictx{args.friction_x}_fricty{args.friction_y}_temp{args.temperature}.p","wb") as f:
        pickle.dump([traj_int3, traj_int3neg], f)
