...
parser.add_argument("-n_steps", "--n_steps", dest="n_steps",
                    default=5000000, type=int, help="Number of steps")
...
taus = np.arange(10,traj.shape[0]-1,40*5,dtype=int)

TEs_Z_to_X = np.zeros((len(taus)))
TEs_Z_to_Y = np.zeros((len(taus)))
TEs_Z_to_X_givenY = np.zeros((len(taus)))
TEs_Z_to_Y_givenX = np.zeros((len(taus)))
...
for i_tau, tau in tqdm(enumerate(taus)):
    ...
    # compute (continuous) bivariare transfer entropy in both directions
    TEs_Z_to_X[i_tau] = (
        knncmi.cmi(['z_0'], ['x_tau'], ['x_0'], k=args.k, data=dataset, minzero=args.minzero)
    )
    TEs_Z_to_Y[i_tau] = (
        knncmi.cmi(['z_0'], ['y_tau'], ['y_0'], k=args.k, data=dataset, minzero=args.minzero)
    )

    # compute (continuous) conditional transfer entropy in both directions
    TEs_Z_to_X_givenY[i_tau] = (
        knncmi.cmi(['z_0'], ['x_tau'], ['x_0','y_0'], k=args.k, data=dataset, minzero=args.minzero)
    )
    TEs_Z_to_Y_givenX[i_tau] = (
        knncmi.cmi(['z_0'], ['y_tau'], ['y_0','x_0'], k=args.k, data=dataset, minzero=args.minzero)
    )

# save data
pickle.dump([taus, TEs_Z_to_X, TEs_Z_to_Y, TEs_Z_to_X_givenY, TEs_Z_to_Y_givenX], 
            open(f"./pickles_te_multiv_ZtoXY/seed{args.seed}_N{args.N}_k{args.k}.p","wb"))