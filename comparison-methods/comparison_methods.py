...
def compute_cross_mapping(X_time_delay, Y_time_delay):
    ...
    if X_time_delay.shape[1] != Y_time_delay.shape[1]:
        raise ValueError(f"Embedding dimensions must be the same for the two systems!")
    ...
    for neighbor in range(E+1):
        u_j_x[:,neighbor] = np.exp(-distances_x[:,neighbor+1] / distances_x[:,1])
        u_j_y[:,neighbor] = np.exp(-distances_y[:,neighbor+1] / distances_y[:,1])
    ...
    for i_time in range(L):
        for neighbor in range(E+1):
            X_reconstructed[i_time] += omega_j_y[i_time,neighbor] * X[indices_y[i_time,neighbor+1]]
            Y_reconstructed[i_time] += omega_j_x[i_time,neighbor] * Y[indices_x[i_time,neighbor+1]]
...
def compute_extended_granger_index(X_time_delay, Y_time_delay, n_neighborhoods=200, ks=[100], seed=1998):
    """
    ...
    for i_k in range(len(ks)):
        ...
        for i_neighborhood in range(n_neighborhoods):
            ...
            var_residual_Y_given_XY.append( np.mean( (Ys[:,0] - lr.predict(np.append(Xs[:,1:],Ys[:,1:],axis=-1)))**2 ))

            # single autoregression to predict y using only the past of y
            lr = LinearRegression()
            lr.fit(Ys[:,1:], Ys[:,0])
            var_residual_Y_given_Y.append( np.mean( (Ys[:,0] - lr.predict(Ys[:,1:]))**2 ))

            # (2) Y->X test
            # VAR to predict x using both past of x and y
            lr = LinearRegression()
            lr.fit(np.append(Xs[:,1:],Ys[:,1:],axis=-1), Xs[:,0])
            var_residual_X_given_XY.append( np.mean( (Xs[:,0] - lr.predict(np.append(Xs[:,1:],Ys[:,1:],axis=-1)))**2 ))

            # single autoregression to predict x using only the past of x
            lr = LinearRegression()
            lr.fit(Xs[:,1:], Xs[:,0])
            var_residual_X_given_X.append( np.mean( (Xs[:,0] - lr.predict(Xs[:,1:]))**2 )) 

        EGCI_X_to_Y[i_k] = 1. - np.mean(np.array(var_residual_Y_given_XY) / np.array(var_residual_Y_given_Y))
        EGCI_Y_to_X[i_k] = 1. - np.mean(np.array(var_residual_X_given_XY) / np.array(var_residual_X_given_X))
...
def compute_measure_L(X_time_delay, Y_time_delay, k=5):
    ...
    with the Information Imbalance measure
    ...
    if X_time_delay.shape[0] != Y_time_delay.shape[0]:
        raise ValueError(f"Number of samples must be the same in the two systems!")
    N = X_time_delay.shape[0]

    imbalance_X_to_Y, imbalance_Y_to_X = compute_info_imbalance(X_time_delay, Y_time_delay, 
                                                                k_A=k, k_B=k, metric="euclidean")
