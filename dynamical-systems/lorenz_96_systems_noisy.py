...
def coupled_lorenz_96(xy, t, N, F1, F2, epsilon, sigma):
    """
    ...
    for i in range(N):
        dx[i] = (x[(i + 1) % N] - x[i - 2]) * x[i - 1] - x[i] + F1 + sigma*np.random.normal()
    for i in range(N):
        dy[i] = (y[(i + 1) % N] - y[i - 2]) * y[i - 1] - y[i] + F2 + sigma*np.random.normal()\
                + epsilon*x[i]
...
if __name__ == '__main__':
    ...
    trajectory = odeint(coupled_lorenz_96, xy0, times,
                        args=(args.N, args.F1, args.F2, args.epsilon, sigma))
