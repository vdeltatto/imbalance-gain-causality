...
def coupled_lorenz_96(xy, t, N, F1, F2, tau, epsilon):
    """
    ...
    for i in range(N):
        dx[i] = 1.0/tau * ((x[(i + 1) % N] - x[i - 2]) * x[i - 1] - x[i] + F1)
    for i in range(N):
...
if __name__ == '__main__':
    ...
    parser.add_argument("-tau", "--tau", dest="tau",
                        ...
                        help="Value of tau parameter, tuning speed of X")
    ...
    trajectory = odeint(coupled_lorenz_96, xy0, times,
                        args=(args.N, args.F1, args.F2, args.tau, args.epsilon))
