...
transfer_entropy_X_to_Y = np.zeros(len(taus_eq))
transfer_entropy_Y_to_X = np.zeros(len(taus_eq))

for itau_eq, tau_eq in enumerate(taus_eq):
    trajectory = pickle.load(open(f"../trajs/seed{args.seed}_itau{itau_eq}.p","rb"))
...
pickle.dump(
    ...
    open(f"./pickles/seed{args.seed}_k{args.k}_minzero{bool(args.minzero)}.p","wb")
)