...
for ieps, eps in enumerate(epsilons):
    trajectory = pickle.load(open(f"../fig3_errors/trajs/lorenz/seed{seed}_ieps{ieps}.p","rb"))
    ...
    Xtau = trajectory[sample_times+tau,1].reshape((-1,1))  # N.B. single time-points at time tau, not windows!
...
pickle.dump([info_imbalances_X_to_Y, info_imbalances_Y_to_X], open(f"./pickles/lorenz/seed{seed}.p","wb"))