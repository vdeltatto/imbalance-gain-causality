...
for ieps, eps in enumerate(epsilons):
    trajectory = pickle.load(open(f"../fig3_errors/trajs/lorenz96/seed{seed}_ieps{ieps}.p","rb"))
    ...
    Xtau = trajectory[sample_times,1].reshape((-1,1)) # N.B. single time-points at time tau, not windows!
    Ytau = trajectory[sample_times,D+1].reshape((-1,1))
