...
if args.case == "XY":
    ...
    for i_tau, tau in enumerate(taus):
        ...
        Ytau = trajectories[:,t0+tau-E:t0+tau, 2]
    ...
    pickle.dump([taus, info_imbalances_X_to_Y, info_imbalances_Y_to_X], open(f"./pickles_{args.coupling}/XY_seed{args.seed}.p","wb"))

elif args.case == "ZX":

    info_imbalances_Z_to_X = np.zeros((len(taus),len(alphas)))
    info_imbalances_X_to_Z = np.zeros((len(taus),len(alphas)))
    ...
    for i_tau, tau in enumerate(taus):
        ...
        info_imbalances_Z_to_X[i_tau] = d.return_inf_imb_causality(
            cause_present=Z0, effect_present=X0, effect_future=Xtau, weights=alphas, k=k)
        info_imbalances_X_to_Z[i_tau] = d.return_inf_imb_causality(
            cause_present=X0, effect_present=Z0, effect_future=Ztau, weights=alphas, k=k)

    pickle.dump([taus, info_imbalances_Z_to_X, info_imbalances_X_to_Z], open(f"./pickles_{args.coupling}/ZX_seed{args.seed}.p","wb"))
    ...
    pickle.dump([taus, info_imbalances_Z_to_Y, info_imbalances_Y_to_Z], open(f"./pickles_{args.coupling}/ZY_seed{args.seed}.p","wb"))