...
def compute_info_imbalance_causality(X0, Y0, rank_matrix_Ytau, alpha=1.0, k=1, metric="euclidean"):
    """
    ...
    if X0.shape[0] != Y0.shape[0]:
        raise ValueError(f"Number of points must be the same in X(0) and Y(0)!")
    N = X0.shape[0]
...
def scan_alphas(cause_present, effect_present, rank_matrix_effect_future, alphas, k=1, n_jobs=1, metric="euclidean"):
    """
    ...
    info_imbalances = Parallel(n_jobs=n_jobs)(delayed(compute_info_imbalance_causality)(
                                              X0=cause_present, Y0=effect_present, rank_matrix_Ytau=rank_matrix_effect_future, 
                                              alpha=alpha, k=k, metric=metric) for alpha in alphas)
