...
# shuffle indices of times and genes datasets 
new_indices = np.random.choice(np.arange(times.shape[0]), replace=False, size=times.shape[0])
times = times[new_indices]
genes = genes[new_indices]

# cross-validation
N_fold = times.shape[0] // 3
...
target_ranks_times = nns_index_array(times[train_indices].reshape(-1,1), maxk=len(train_indices)-1)
d = MetricComparisons(genes[train_indices], njobs=njobs)
variables_train, imbalances_train, _ = d.greedy_feature_selection_target(target_ranks=target_ranks_times, 
                                                            n_best=args.n_best,
                                                            n_coords=args.n_coords,
                                                            k=args.k,
                                                            symm=False)

# compute greedy imbalance on val set
#target_ranks_times = nns_index_array(times[val_indices].reshape(-1,1), maxk=len(val_indices)-1)
#d = MetricComparisons(genes[val_indices], njobs=njobs)
#variables_val, imbalances_val, _ = d.greedy_feature_selection_target(target_ranks=target_ranks_times, 
#                                                            n_best=args.n_best,
#                                                            n_coords=args.n_coords,
#                                                            k=args.k,
#                                                            symm=False)
...
# first validation
target_ranks_times = nns_index_array(times[val_indices1].reshape(-1,1), maxk=len(val_indices1)-1)
d = MetricComparisons(genes[val_indices1], njobs=njobs)
imbalances_val1 = d.return_inf_imb_target_selected_coords(target_ranks=target_ranks_times, 
                                                              coord_list=variables_train, 
                                                              k=args.k)

target_ranks_times = nns_index_array(times[val_indices2].reshape(-1,1), maxk=len(val_indices2)-1)
d = MetricComparisons(genes[val_indices2], njobs=njobs)
imbalances_val2 = d.return_inf_imb_target_selected_coords(target_ranks=target_ranks_times, 
                                                              coord_list=variables_train, 
                                                              k=args.k)
