...
def compute_info_imbalance(data_A, data_B, k_A=1, k_B=1, metric="euclidean"):
    ...
    if data_A.shape[0] != data_B.shape[0]:
        raise ValueError(f"Number of points must be the same in the two representations!")
    N = data_A.shape[0]
...
def construct_time_delay_embedding(X, Y, E, tau_e, sample_times=None):
    ...
    """
    if len(X) != len(Y):
        raise ValueError(f"Number of points must be the same in the two trajectories!")
    ...
    if sample_times is None:
        ...
        for i_sample in range(N):
            embedding_times = np.arange(sample_times[i_sample], sample_times[i_sample]-tau_e*E, -tau_e)
